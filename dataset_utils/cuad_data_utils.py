...
master_csv_savepath = os.path.join(
    cuad_base_path, "cuad_master_csv_mapping.csv")
...
def get_cuad_fnames(extensions,
                    search_path,
                    cuad_base_path,
                    is_exists,
                    master_df,
...
def fetch_filelists(master_df, is_exists):
    ...
    print(
        f"Fetching filepaths for PDF files. Num Extensions: {len(pdf_extensions)}\n")
    pdf_paths = get_cuad_fnames(pdf_extensions,
                                cuad_pdf_path,
                                ".",
                                is_exists,
                                master_df,
    ...
    print(
        f"Fetching filepaths for txt files. Num Extensions: {len(txt_extensions)}\n")
    txt_paths = get_cuad_fnames(txt_extensions,
                                cuad_txt_path,
                                ".",
                                is_exists,
                                master_df,
    ...
    raw_html_paths = get_cuad_fnames(raw_html_extensions,
                                     cuad_raw_html_path,
                                     ".",
                                     is_exists,
                                     master_df,
...
def match_filepath_to_contract(master_df, filepaths, col_name):
    ...
    if mismatch_count:
        ...
        print("Number of files that were not matched " +
...
def create_cuad_master_csv(pdf_paths,
                           txt_paths,
                           raw_html_paths,
                           is_exists,
                           master_df):
    ...
    df = pd.DataFrame()
    df = df.assign(contract_uid=None,
                   contract_type=None,
                   dataset=None,
    ...
    if is_exists:
        ...
        contract_uids = list(range(last_contract_uid_int,
                                   last_contract_uid_int + len(pdf_paths)
                                   )
                             )
    ...
    df['contract_type'] = df['pdf_path'].apply(
        lambda row: row.split("/", 4)[-2])
    ...
    master_df = match_filepath_to_contract(
        master_df, raw_html_paths, 'raw_html_path')
...
def rename_files(row, col_name):
    ...
    if isinstance(old_path, str):
        ...
        updated_path = os.path.join(
            old_path_parent_dir, f"{contract_uid}{file_ext}")
...
def main():
    ...
    pdf_paths, txt_paths, raw_html_paths = fetch_filelists(master_df,
                                                           is_exists)

    # create and populate master csv. Finally save the mapping to disk
    master_df = create_cuad_master_csv(pdf_paths,
                                       txt_paths,
                                       raw_html_paths,
                                       is_exists,
                                       master_df)

...
def get_toc_page_id(linewise_ocr_output):
    ...
    regex_exp = (
        r"(table of contents|tableof(?:contents)?|(?:table\s)?of*conten|contents?)"
    )

    # loop through the linewise ocr output and try to find a match
    for page_id, line in zip(
        linewise_ocr_output["page_id"], linewise_ocr_output["full_line"]
    ):
...
def match_toc_label_with_ocr_seg(
    toc_label,
    linewise_ocr_output,
    idx1,
    idx2,
    ...
    if idx2 is not None:
        ...
        multi_line_text = (
            linewise_ocr_output["full_line"][idx1]
        ...
        first_line_match_cond = first_line_match_score >= first_line_match_threshold

        if (
            full_text_match_cond
            and match_len_cond
            and beg_line_match_cond
            and first_line_match_cond
        ):
            ...
            first_2words_match_score = fuzz.partial_ratio(
                toc_label_beg.lower(), matched_line_beg.lower()
            )

            first_2words_match_cond = first_2words_match_score >= subset_match_threshold
            if first_2words_match_cond:
                ...
                bboxes = [
                    linewise_ocr_output["bboxes"][idx1],
    ...
    # the matched text segment must have length at least some %-age of
    # the toc label it matched with
    match_len_cond = len(line_text) >= (len(toc_label[0]) * line_len_match_threshold)
...
def find_single_match_id(
    ...
    itertuple = zip(
        range(prev_line_pointer, len(linewise_ocr_output["full_line"])),
        range(prev_line_pointer + 1, len(linewise_ocr_output["full_line"])),
    )

    for idx1, idx2 in itertuple:

        # first try matching with first line
        match_info, prev_line_pointer = match_toc_label_with_ocr_seg(
            toc_label,
            linewise_ocr_output,
            idx1,
            None,
...
def find_all_match_ids(toc_match_config, toc_label_dict, linewise_ocr_output):
    ...
    for _, toc_label in toc_label_dict.items():
        match_info, prev_line_pointer = find_single_match_id(
            toc_label,
            linewise_ocr_output,
            subset_match_threshold,
            line_len_match_threshold,

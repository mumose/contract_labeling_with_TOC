...
def save_pipeline_outputs(
    match_metadata_df, pipeline_output_json, pipeline_results_dir, contract_uid
):
    ...
    match_metadata_savepath = os.path.join(
        contract_results_dir, f"{contract_uid}_match_metadata.csv"
    )

    match_metadata_df.to_csv(match_metadata_savepath, index=False)

    # save the json format of the pipeline results
    json_savepath = os.path.join(contract_results_dir, f"{contract_uid}.json")
...
def run_matching_single_contract(row, pipeline_config):
    ...
    ***************************************************************************
    ...
    toc_label_dict, html_out_savepath = fetch_html.process_single_html(
    ...
    ***************************************************************************
    ...
    ocr_results_path = os.path.join(cuad_base_dir, row["ocr_results_path"])
    ...
    linewise_ocr_output = postprocess_ocr_output.get_linewise_ocr_output(
        raw_ocr_results, iou_threshold
    )
    ...
    linewise_ocr_output = linewise_ocr_output_df.sort_values(
        by=["page_id", "ymin_min"]
    ).to_dict(orient="list")
    ...
    ***************************************************************************
    ...
    match_metadata = match_toc_labels.find_all_match_ids(
        toc_match_config, toc_label_dict, linewise_ocr_output
    )
    ...
    (
        ...
        match_metadata_df["exact_match_text"],
    ) = zip(
        *match_metadata_df.apply(
            lambda row: merge_bboxes.extract_exact_match(row), axis=1
        )
    )
    ...
    ***************************************************************************
    """
    pipeline_output_json = visualize_results.viz_results_single_contract(
        pipeline_config, match_metadata_df, contract_uid
    )
    ...
    ***************************************************************************
    """
    match_metadata_savepath, json_savepath = save_pipeline_outputs(
        match_metadata_df, pipeline_output_json, pipeline_results_dir, contract_uid
    )
...
def main(input_df, pipeline_config):
    ...
    for idx, (row_idx, row) in tqdm(enumerate(filter_df.iterrows())):
        match_metadata_savepath, json_savepath = run_matching_single_contract(
            row, pipeline_config
        )

        # update the input df
        input_df.loc[row_idx, "pipeline_results_csv_path"] = match_metadata_savepath

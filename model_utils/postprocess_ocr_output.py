...
def get_line_ids_to_merge(word_level_ocr_output, threshold):
    ...
    for i in range(len(word_level_ocr_output['words'])):
        ...
        for j in range(i + 1, len(word_level_ocr_output['words'])):
            page_id_cond = (word_level_ocr_output['page_id'][i] ==
                            word_level_ocr_output['page_id'][j])

            if (
                 page_id_cond and
                 getIOU(word_level_ocr_output['ymin_ymax'][i],
...
def get_linewise_ocr_output(raw_ocr_output, threshold):
    ...
    for i in range(len(word_level_ocr_output['words'])):
        if i not in line_merge_map and i not in follow_merge_map:
            linewise_ocr_output['full_line'].extend(
                [word_level_ocr_output['words'][i]])

            linewise_ocr_output['bboxes'].extend(
                [word_level_ocr_output['bboxes'][i]])
    ...
    for idx, (min_element, max_element) in \
        enumerate(zip(linewise_ocr_output['ymin_min'],
                  linewise_ocr_output['ymax_max'])):
